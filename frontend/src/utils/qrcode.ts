...
class QRCodeModel {
  ...
  static createData = function (typeNumber: number, errorCorrectLevel: number, dataList: QR8bitByte[]) {
  ...
  makeImpl(test: unknown, maskPattern: number) {
  ...
  setupTypeInfo(test: unknown, maskPattern: number) {
...
class QRRSBlock {
  ...
  static getRSBlocks = function (typeNumber: number, errorCorrectLevel: number) {
  ...
  static getRsBlockTable = function (typeNumber: number, errorCorrectLevel: number) {
...
class QRCode {
   ...
   */
  async makeCode(sText: string, options: { logo?: string } = {}) {
